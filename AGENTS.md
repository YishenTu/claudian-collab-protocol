...
- The current source contract is `4.2.1`, canonical wire v9, Cloud binding v5, authority-transfer/export coordination format v1, and offline-backup coordination format v3. Source readiness does not establish registry publication; consumers wait for verified release evidence.
...
- Do not publish a protocol release when only Claudian or Cloud Server implementation changed. A compatible protocol correction uses the next `4.2.x` version unless the user explicitly approves a different release line.
