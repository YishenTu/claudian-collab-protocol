...
assert.equal(packageVersion, '5.1.0');
assert.equal(
...
const esmOutput = run(process.execPath, [
  ...
  "import { COLLAB_CLOUD_BINDING_VERSION, COLLAB_PROTOCOL_VERSION, collabMemberRef, parseCollabTicketReferences } from '@claudian-collab/protocol';"
    ...
    + " if (!resolved.endsWith('/dist/esm/index.mjs') || COLLAB_PROTOCOL_VERSION !== 15 || COLLAB_CLOUD_BINDING_VERSION !== 10 || collabMemberRef('member_1') !== 'refs/heads/members/member_1' || references.status !== 'ok' || references.references[0]?.ticketNumber !== 3) process.exit(1);"
