...
- `3.2.1` and canonical wire v6 are the current published baseline. The approved next contract release is exactly `3.3.0`: additive Cloud Project-membership operations keep wire v6, additive capability tokens keep Cloud binding v2, and the pre-production offline backup contract is replaced by coordination v3.
- `CollabProjectCheckpoint` owns wire-visible authority-transfer/export coordination format v1 and remains byte-for-byte unchanged in `3.3.0`. `CollabProjectBackupCheckpoint` owns offline backup coordination. Until the first production deployment is explicitly declared, an explicitly approved backup-format replacement may use a package minor release without a wire or Cloud-binding increase only when the executable classifier proves the v1 module unchanged and every unrelated public change satisfies its normal classification. The replacement removes the prior decoder and fixtures rather than retaining a legacy path. At first production launch, replace this bounded exception with a stable backup compatibility obligation before any later backup-format change.
...
- Do not publish a protocol release when only Claudian or Cloud Server implementation changed. After the approved `3.3.0` release, a compatible protocol correction uses the next `3.3.x` version unless the user explicitly approves a different release line.
