...
function record(overrides = {}) {
  return createReleaseRecord({
    ...
    packageManifest,
    packResult: {
      filename: 'claudian-collab-protocol-4.3.3.tgz',
      files: [
...
test('derives the complete package inventory from nested source modules', t => {
  const directory = mkdtempSync(path.join(os.tmpdir(), 'protocol-source-inventory-'));
  t.after(() => rmSync(directory, { recursive: true, force: true }));
  mkdirSync(path.join(directory, 'core'));
  writeFileSync(path.join(directory, 'index.ts'), "export type { Value } from './core/types';");
  writeFileSync(path.join(directory, 'core/types.ts'), 'export type Value = string;');
  writeFileSync(path.join(directory, 'core/AGENTS.md'), '# Core constraints');
  assert.deepEqual(expectedPackedPaths(directory), [
    'LICENSE',
    'README.md',
    'dist/core/types.d.ts',
    'dist/core/types.js',
    'dist/esm/core/types.mjs',
    'dist/esm/index.mjs',
    'dist/index.d.ts',
    'dist/index.js',
    'package.json',
...
test('verifies the exact reviewed tarball before a clean consumer can install it', () => {
  ...
  try {
    const tarballPath = path.join(directory, 'claudian-collab-protocol-4.3.3.tgz');
