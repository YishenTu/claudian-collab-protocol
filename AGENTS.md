...
- The current source contract is `4.1.2`, canonical wire v8, Cloud binding v4, authority-transfer/export coordination format v1, and offline-backup coordination format v3. Source readiness does not establish registry publication; consumers wait for verified release evidence.
...
- A relinquished Cloud-to-LAN backup retains exactly two transfer verification keys: the target evidence-pinned receipt key and one Cloud source proof verifier. The format bounds this structural allowance; clean restore cryptographically identifies the sole key that verifies the relinquishment proof.
